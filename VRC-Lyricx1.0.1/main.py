...
configuration.user_agent = "VRC-Lyricx/1.0.1 (contact: goatgood88@gmail.com)"
...
os.environ["waitForLyricsToEnd"] = "0.0"
        ...
        if spotifyWindowTitle != lastSpotifyWindowTitle: 
            ...
            try: 
                if SpotifyRequ["is_playing"] == True:
                    ...
                    print(float(os.environ["waitForLyricsToEnd"]))
                    print(datetime.now())
                    sleep(float(os.environ["waitForLyricsToEnd"]))
                    print(datetime.now())
                    print("Starting lyrics thread")
                    dt = datetime.now()
                    delayLyricsBy = (float(f"{dt.second}.{dt.microsecond}")-startProcessTime)+float(os.environ["waitForLyricsToEnd"])
...
def lyrics(showLyrics, songUrl:str, lastStartTimeMs:float, delayLyricsBy: float):
    ...
    try:
        for lyric in range(len(lyricsRequJson["lines"])):
            os.environ["waitForLyricsToEnd"] = str(((float(lyricsRequJson["lines"][lyric+1]["startTimeMs"])-float(lyricsRequJson["lines"][lyric]["startTimeMs"]))/1000)+1)
